...
$sql = "SELECT category as `Disaster`, disaster_date as `Disaster Date`, sum(quantity) as `total quantity`, sum(cost) as `total cost` FROM food, relief, disaster where relief.disasterid = disaster.DISASTERID and food.RELIEFID = relief.RELIEFID group by food.RELIEFID";
