...
$sql = "SELECT category as `Disaster`, disaster_date as `Disaster Date`, sum(quantity) as `total quantity`, sum(cost) as `total cost` FROM cloth, relief, disaster where relief.disasterid = disaster.DISASTERID and cloth.RELIEFID = relief.RELIEFID group by cloth.RELIEFID";
